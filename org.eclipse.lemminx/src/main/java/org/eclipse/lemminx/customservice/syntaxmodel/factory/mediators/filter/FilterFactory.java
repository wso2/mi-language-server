...
import org.eclipse.lemminx.customservice.syntaxmodel.pojo.mediator.filter.filter.Filter;
import org.eclipse.lemminx.customservice.syntaxmodel.pojo.mediator.filter.filter.FilterElse;
import org.eclipse.lemminx.customservice.syntaxmodel.pojo.mediator.filter.filter.FilterThen;
...
public class FilterFactory extends AbstractMediatorFactory {
    ...
    private STNode createThenElse(DOMNode child) {
        ...
        if (children != null && !children.isEmpty()) {
            mediators = Utils.createMediators(children);
        }
